...
# normal vgg-m model
__models['vgg-m'] = (lambda io, model_name='low-vgg-m', 
...
__models['low-vgg-m'] = (lambda io, model_name='low-vgg-m', 
    path='': _low_vgg_m_gen(io=io, model_name=model_name, path=path))

# vgg-16 low-rank model 
__models['low-vgg-16'] = (lambda io, model_name='low-vgg-16', 
    path='': _low_vgg_16_gen(io=io, model_name=model_name, path=path))
...
def _low_vgg_m_gen(io, model_name, path):
...
def _low_vgg_16_gen(io, model_name, path):
